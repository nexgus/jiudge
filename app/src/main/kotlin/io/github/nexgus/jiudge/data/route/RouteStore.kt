 ...
 */
class RouteStore(private val dir: File) {
    ...
    fun list(): List<Summary> {
        val files = dir.listFiles { f -> f.isFile && f.name.endsWith(PlannedRoute.FILE_SUFFIX) }
            ?: return emptyList()
    ...
    private fun slug(name: String): String =
        name.trim().ifEmpty { "route" }.replace(Regex("[^\\p{L}\\p{N}_-]+"), "_").take(40)
}
